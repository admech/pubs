...
Последнее слагаемое -- $\sum_{i=1}^{N}\dot{\chi}_i^*\M^*_i\vnu$ -- является суммой векторов размерности $K$, где $K = \dim\vnu$ -- число степеней свободы системы. Компоненты этих векторов с номерами $\alpha = 4,\dots, K$, то есть компоненты, соответствующие свободным роликам, имеют вид $\dot{\chi}_i^*\nu_3B\Lambda^{-1}\cos\chi_{ij},$ где индексы $i,j$ связаны с индексом $\alpha$ по формуле $\alpha = 3 + s(i, j)$ (см. формулу\upr{eq:num}).

Для величин $P_\mu$ во втором слагаемом левой части~\ref{Tatarinov}, подставляя в их определение~\ref{eq:tatimp} выражения обобщенных скоростей через псевдоскорости~\ref{eq:constraints_V} и применяя метод неопределенных коэффициентов, получим выражения:
...
Для завершения вывода уравнений движения рассмотрим правую часть равенства (\ref{Tatarinov}). Она отлична от нуля лишь при $\alpha = 1,\ 2,\ 3$. Сперва выразим импульсы $\vec{p}$ через псевдоскорости $\vnu$. Обозначим
$$
...
\begin{enumerate}[wide]
    ...
    \item При $B = 0$ (ролики не имебт инерции) все слагаемые в правой части равенства~(\ref{eq:full_system}), кроме первого, обращаются в нуль, как и все члены, соответствующие свободным роликам, в его левой части (см. Приложение, равенства\upr{mstar}). В этом случае существенными остаются лишь первые три уравнения системы\upr{Tatarinov} относительно $\nu_1,\ \nu_2,\ \nu_3$. Причем в этом случае оставшиеся нетривиальные уравнения в точности совпадают с уравнениями движения безынерционной модели экипажа \cite{ZobovaTatarinovPMM}.
